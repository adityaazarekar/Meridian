...
def compute_metrics(info: dict, history_df) -> dict:
    ...
    # yfinance already returns dividendYield and average yield as whole percentages (e.g. 4.31 for 4.31%)
    div_yield = safe_round(div_yield_raw if div_yield_raw is not None else None)
    payout_pct = safe_round(payout_ratio * 100 if payout_ratio is not None else None)
    div_yield_5y = safe_round(five_y_div if five_y_div is not None else None)
